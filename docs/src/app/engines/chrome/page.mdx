...
When `--args` includes a flag with the same key as a built-in default, your value wins for that key.
For example, `--disable-sync=false` overrides the default `--disable-sync`.
